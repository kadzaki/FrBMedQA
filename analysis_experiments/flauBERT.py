...
class SciBertReaderMax(nn.Module):
    def __init__(self, frozen_top):
        ...
        self.tok = FlaubertTokenizer.from_pretrained('flaubert/flaubert_base_uncased', do_lowercase=True, additional_special_tokens=['[MASK]', '[SEP]', '[CLS]', '@entity'])
        self.bert = FlaubertModel.from_pretrained('flaubert/flaubert_base_uncased')
...
# Load checkpoint
resume_from = 'results/flaubert/flaubert_checkpoint_max.pth.tar'
resumed = False
...
for epoch in range(start_epoch, max_epochs):
    out_f = open('results/flaubert/flaubert_max.txt', 'a+')
    print()
    out_f.write('\n')
    print('-'*30)
    out_f.write('-'*30 + '\n')
    ...
    torch.save(state, 'results/flaubert/flaubert_checkpoint_max.pth.tar')
    out_f = open('results/flaubert/flaubert_max.txt', 'a+')
    # Evaluation Dev
    print()
    out_f.write('\n')
    ...
    if dev_acc > best_dev_acc:
        ...
        torch.save(state, 'results/flaubert/flaubert_best_checkpoint_max.pth.tar')
    else:
    ...
    if do_test:
        out_f = open('results/flaubert_max.txt', 'a+')
        # Evaluation Test
        print()
        out_f.write('\n')
