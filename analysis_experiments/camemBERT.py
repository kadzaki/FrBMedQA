...
class SciBertReaderMax(nn.Module):
    def __init__(self, frozen_top):
        ...
        self.tok = CamembertTokenizer.from_pretrained('camembert-base', do_lowercase=True, additional_special_tokens=['[MASK]', '[SEP]', '[CLS]', '@entity'])
...
# Load checkpoint
resume_from = 'results/camembert/camembert_checkpoint_max.pth.tar'
resumed = False
...
for epoch in range(start_epoch, max_epochs):
    out_f = open('results/camembert/camembert_max.txt', 'a+')
    print()
    out_f.write('\n')
    print('-'*30)
    out_f.write('-'*30 + '\n')
    ...
    torch.save(state, 'results/camembert/camembert_checkpoint_max.pth.tar')
    out_f = open('results/camembert/camembert_max.txt', 'a+')
    # Evaluation Dev
    print()
    out_f.write('\n')
    ...
    if dev_acc > best_dev_acc:
        ...
        torch.save(state, 'results/camembert/camembert_best_checkpoint_max.pth.tar')
    else:
